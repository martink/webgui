...
sub toHtml {
    ...
    my $output = '<select name="' . $self->get('name') . '">';
    my $iterator = WebGUI::Graphics::Font->getAllIterator( $self->session );
    while ( my $font = $iterator->() ) {
        my $style = "width: 200px; height: 50px; background: url(". $font->getPreviewUrl. "); no-repeat";

        $output .= qq{<option value="} . $font->getId . qq{" style="$style">&nbsp;</option>};

        $options->{ $font->getId } = $font->get('name'); #' . $font->getPreviewUrl . '" />';
    }
    $output .= '</select>';
